...
def plot_conf_mat_and_precision_recall(path, gt_labels, pr_labels,
    ...
    for i in range(len(unique_gestures)):
        if i != none_ids:
            mats_labels.append([unique_gestures[i], 'none'])
            mat = np.zeros((2,2), dtype=np.float)
            sum_trash = 0;
            for j in range(len(unique_gestures)):
                if i != j:
                    sum_trash += temp_col_both_hands[j]

                for k in range(len(unique_gestures)):
                    if j == i:
                        if k == i:
                            mat[0][0] += temp_cm[j][k]
                        else:
            ...
            trash = [temp_col_both_hands[i] , sum_trash]

            for j in range(2):
                summa = mat[j][0] + mat[j][1] + trash[j]
                for k in range(2):
                    mat[j][k] = 100.0*mat[j][k] / summa

                trash[j] = 100.0*trash[j] / summa


            trash = np.round(trash, 3)
            mat = np.round(mat, 3)
            mats.append(copy(mat))
            mats_trash.append(trash)
    ...
    for i in range(len(mats)):
        plot_confusion_matrix(mats[i], mats_labels[i], "Hand pose conf mat")
        plt.savefig(path + "_" + str(i) + "_cm.png")
        plt.clf()

    # #print temp_col_both_hand   s
...
def make_html_page(path, add_joints_whole, add_joints, add_centers,
    ...
    html_text += add_button("Confusion matrix", "img/whole_0_cm.png")
    html_text += add_button("Confusion matrix", "img/whole_1_cm.png")
    html_text += add_button("Confusion matrix", "img/whole_2_cm.png")
    html_text += add_button("Confusion matrix", "img/whole_3_cm.png")
    html_text += add_button("Confusion matrix", "img/whole_4_cm.png")
    html_text += add_button("Confusion matrix", "img/whole_5_cm.png")
...
def run_evaluation_on_dataset(dataset, add_mispredictions):
    ...
    if add_mispredictions:
        ...
        print_mispredictions(paths, hand_pose_misclassifications, gt_labels,
                             pr_labels, os.path.join(dattaset, "img"),
